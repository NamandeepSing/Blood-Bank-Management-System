...
export default function GetRequestId() {
  const [query, setQuery] = useState({ patient_name: '', contact_info: '' });
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    if (!query.patient_name.trim() && !query.contact_info.trim()) {
      setError('Please enter either a patient name or contact information');
      return;
    }
    ...
    try {
      const res = await axios.get('/api/request/id', { params: query });
      setResult(res.data);
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to retrieve request data');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 max-w-2xl mx-auto">
      <div className="bg-gradient-to-r from-green-600 to-green-500 text-white rounded-t-xl p-6">
        <h2 className="text-2xl font-serif">Get Request ID</h2>
        <p className="text-green-100 mt-1">Retrieve blood request information by patient details</p>
      </div>
      ...
      <div className="p-6">
        ...
        {result && (
          <div className="mt-6">
            <h3 className="text-lg font-medium text-gray-800 mb-4 border-b border-gray-200 pb-2">Request Information</h3>
            ...
            {Array.isArray(result) ? (
              result.length > 0 ? (
                ...
                <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 p-4 rounded-lg">
                  No request records found matching your criteria.
                </div>
              )
            ) : (
